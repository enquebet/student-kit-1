...
export default function CapacitorSeriesCalculatorTool() {
  ...
  return (
    <ToolShell title="Series Capacitor Calculator" description="Calculate the total equivalent capacitance of multiple capacitors connected in series." category="engineering" seoTitle="Series Capacitor Calculator | Equivalent Capacitance | StudentKit" seoDescription="Free online calculator to find the total equivalent capacitance of capacitors in series. Just enter the values and get instant results.">
      <div className="max-w-2xl mx-auto space-y-8">
            ...
            {capacitors.map((c, index) => (
              <div key={c.id} className="flex items-center gap-4">
                ...
                <div className="flex-1 flex items-center gap-2">
                  <input type="number" value={c.value} onChange={(e) => updateCapacitor(c.id, e.target.value)} className="flex-1 p-2 border rounded-md" placeholder={`Capacitor ${index + 1} value`} min="0.001" step="any" />
        ...
        <div className="prose max-w-none bg-white p-8 rounded-xl border">
          ...
          <p>When capacitors are connected in series, the total equivalent capacitance decreases. The formula is identical to the formula for resistors in <strong>parallel</strong>.</p>
          <div className="bg-gray-50 p-6 rounded-lg border my-6 text-center">
             <div className="text-xl md:text-2xl font-bold text-gray-800 font-mono">1/C<sub>eq</sub> = 1/C<sub>1</sub> + 1/C<sub>2</sub> + ... + 1/C<sub>n</sub></div>
          </div>
          <p><strong>Note:</strong> The total equivalent capacitance of a series circuit is always <em>less</em> than the capacitance of the smallest individual capacitor in the series network. However, connecting capacitors in series increases the total working voltage rating.</p>
        </div>
      </div>
    </ToolShell>
  );
