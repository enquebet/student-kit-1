...
export default function SeriesResistorCalculatorTool() {
  ...
  return (
    <ToolShell title="Series Resistor Calculator" description="Calculate the total equivalent resistance of multiple resistors connected in series." category="engineering" seoTitle="Series Resistor Calculator | Equivalent Resistance | StudentKit" seoDescription="Free online calculator to find the total equivalent resistance of resistors in series. Just enter the values and get instant results.">
      <div className="max-w-2xl mx-auto space-y-8">
            ...
            {resistors.map((r, index) => (
              <div key={r.id} className="flex items-center gap-4">
                ...
                <div className="flex-1 flex items-center gap-2">
                  <input type="number" value={r.value} onChange={(e) => updateResistor(r.id, e.target.value)} className="flex-1 p-2 border rounded-md" placeholder={`Resistor ${index + 1} value`} />
        ...
        <div className="prose max-w-none bg-white p-8 rounded-xl border">
          ...
          <p>When resistors are connected in a series, the current that flows through each resistor is the same. The total equivalent resistance (R<sub>eq</sub>) is simply the sum of the individual resistance values.</p>
          <div className="bg-gray-50 p-6 rounded-lg border my-6 text-center">
             <div className="text-2xl font-bold text-gray-800 font-mono">R<sub>eq</sub> = R<sub>1</sub> + R<sub>2</sub> + ... + R<sub>n</sub></div>
          </div>
          <p><strong>Note:</strong> The total resistance of a series circuit is always greater than the resistance of the largest individual resistor in the circuit.</p>
        </div>
      </div>
    </ToolShell>
  );
